...
if __name__ == "__main__":
    ...
    idmapper = build_ena_db.IDMapper(database_params, args.db_name)
    if not idmapper:
    ...
    glob_subdirs_futures = client.map(build_ena_db.glob_subdirs, args.ena_paths)
    ...
    for finished_task in tasks_completed:
        ...
        if not results[1]:
            ...
            new_futures = [client.submit(build_ena_db.glob_files, subdir) for subdir in results[1]]
            ...
            new_futures = [client.submit(build_ena_db.process_many_files, shard, database_params = database_params, db_name = args.db_name, common_output_dir = args.output_dir) for shard in shards if shard]
