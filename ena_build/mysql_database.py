...
class IDMapper:
    ...
    def reverse_lookup(self, foreign_ids: list, batch_size: int = 1000):
        ...
        # this search is grabbing the  uniprot_id from the table if the row's 
        # foreign_id is in the list of foreign_ids (input).
        placeholder = ", ".join(["%s"]*len(foreign_ids))
        sql = f"SELECT uniprot_id FROM idmapping WHERE foreign_id IN ({placeholder})"
